...
class MedicationKnowledge(Document):
	def fhir_object(self):
		fhir_schema = {
			...
			"doseForm": {"coding": [get_code("Medication Form", self.get("doseform"))]} if self.get("doseform") else "",
			"ingredient": self.get_ingredient() if self.get("ingredient") else [],
			"amount": {'value': self.amount},
			"synonym": [self.synonym],
			"productType": [{'text': self.producttype}],
			"preparationInstruction": self.preparationinstruction
		}
	...
	def before_insert(self):
		if not self.fhir_serverid:
			...
			response = requests.post(
				lafia_base_url + '/MedicationKnowledge', json=fhir_schema)
	...
	def on_update(self):
		...
		response = requests.put(
            lafia_base_url + '/MedicationKnowledge/{0}'.format(self.fhir_serverid),
			json=fhir_schema)
	...
	def on_trash(self):
		response = requests.delete(
            lafia_base_url + '/MedicationKnowledge/' + self.fhir_serverid)
