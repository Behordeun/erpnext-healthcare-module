...
class Person(Document):
	def fhir_object(self):
		fhir_schema = {
			...
			"link": [{
				"target": get_reference(self.link_type,self.link) if self.link else ""
			}]
		}
		return fhir_schema
	...
	def on_update(self):
		...
		response = requests.put(
            lafia_base_url + '/fhir/Person/' + self.fhir_serverid,
			json=fhir_schema, verify=False)
	...
	def on_trash(self):
		response = requests.delete(
			lafia_base_url + '/fhir/Person/' + self.fhir_serverid, verify=False)		
		print(response.json())
